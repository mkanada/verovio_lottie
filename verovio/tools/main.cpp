...
int main(int argc, char **argv)
{
    ...
    const std::vector<std::string> outformats
        = { "mei", "mei-basic", "mei-pb", "mei-facs", "svg", "lottie", "dotlottie", "dotlottie-highlight", "midi",
    ...
    if (std::find(outformats.begin(), outformats.end(), outformat) == outformats.end()) {
        std::cerr << "Output format (" << outformat
                  << ") can only be 'mei', 'mei-basic', 'mei-pb', mei-facs', 'svg', 'lottie', 'dotlottie', "
                     "'dotlottie-highlight', 'midi', "
    ...
    if (outformat == "svg") {
    ...
    else if (outformat == "dotlottie-highlight") {
        const int from = page ? *page : 1;
        const int to = allPages ? toolkit.GetPageCount() : from;
        ...
        for (int p = from; p <= to; ++p) {
            ...
            else if (!toolkit.RenderToDotLottieHighlightFile(curOutfile, p)) {
                std::cerr << "Unable to write dotLottie to " << curOutfile << "." << std::endl;
                exit(1);
            }
            else {
                std::cerr << "Output written to " << curOutfile << "." << std::endl;
