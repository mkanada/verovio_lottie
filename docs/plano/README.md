...
| [D01-5](D01-5-bold-italico-tempo.md) | Tarefa paralela: cobrir Bold Italic em texto comum (4º estilo, `LiberationSerif-BoldItalic.ttf`) — reabre o "não coberto" de B03/D01 a pedido do usuário, achado real em Clair de Lune (números de tempo/rubato bold+italic saindo só em negrito) | D01, B03 | reabrir o escopo de B03 (pedido explícito do usuário) | concluído (corpus, junto com D01-6: 0,0135%–0,4153%/média 0,1318% → 0,0135%–0,3946%/média 0,1251%; só Clair de Lune mudou) |
| [D01-6](D01-6-glifo-smufl-em-texto-comum.md) | Tarefa paralela: desenhar como vetor (não `ty:5`) um run de texto comum cujos caracteres são inteiramente códigos SMuFL já cobertos pelas fontes musicais do projeto — achado real em Clair de Lune ("pp" antes de "con sordina", via `<words font-family="Leland Text">` com U+E520 literal) | D01, A08-A10 | — (mecanismo já decidido, só estendendo onde aciona) | concluído (ver "Achado adicional" no doc: o PNG de referência do `compare` para esse glifo específico não é reprodutível — depende de `load_system_fonts()` pegando uma fonte do sistema local não relacionada ao projeto) |
